...
-- here you can see we set the collision to true whenever the player hits something
...
function Level:update(dt)
    ...
    -- This replaces the launch marker if the aliens arent moving 
    if self.launchMarker.launched then
        if not self.launchMarker.splitted then
            local xPos, yPos = self.launchMarker.alien.body:getPosition()
            local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()
            
            -- Close to stop rolling respawn and send them back or end the level
